...
MLE = \hat\Phi = argmax_\Phi p(X|\Phi)
...
- 1차원에서 $$N_1$$개의 샘플이 필요하다면 d차원에서 $$N_1^d$$ㅐ 정도가 있어야 믿을만한 확률 분포를 얻으수 있다. 
...
### 2.3 K-최근접 이웃 추정 (k-nn Estimation)

KDE의 방식 : 고정된 창, 창의 중심 x를 어디에 두느냐에 따라 창 안의 샘플의 개수가 달라진다. 
K-NNE의 방식 : 가변적 창, x를 중심으로 창을 씌우고 k개의 샘플이 창 안에 들어 올때까지 창의 크기를 확장한다. 
...
기본 아이디어는 K-NNE을 활용 : x를 $$\omega_q$$로 분류하라, 이때 $$q=argmax_i k_i$$
